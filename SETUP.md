...
## Стабильный ID устройства (Windows/Linux-агент)

Раньше ID агента был случайным и хранился только в данных установки — при
переустановке/скачивании заново получалась новая запись-дубликат. Теперь ID
выводится из стабильной характеристики самой машины (не IP — он меняется):
Windows — `MachineGuid` из реестра (`HKLM\SOFTWARE\Microsoft\Cryptography`,
читается без прав администратора), Linux — `/etc/machine-id`. Android уже
использовал `ANDROID_ID` по той же причине.

**Разовый эффект перехода**: у уже развёрнутых Windows/Linux-агентов при
первом запуске новой версии ID пересчитается — старая запись останется в
списке устройств как оффлайн-дубликат. Просто "забудьте" её в клиенте
(кнопка ✕ / долгий тап) — новая, стабильная, останется навсегда.
